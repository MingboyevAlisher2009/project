...
export const addGroupImage = async (req, res, next) => {
  try {
    ...
    const date = Date.now();
    const fileName = `uploads/groups/${date}-${req.file.originalname}`;
...
export const deleteGroup = async (req, res, next) => {
  try {
    ...
    if (group.author.toString() !== req.user._id.toString()) {
      return errorResponse(res, 403, "Only the author can delete this group");
    }

    if (userId) {
      group.members.pop(userId);
    ...
    await Attending.deleteMany({ group: id });
